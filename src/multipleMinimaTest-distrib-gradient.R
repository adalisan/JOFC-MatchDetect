...
w.vals<-c(0.1,0.45,0.5,0.55,0.75,0.9,0.985)
w.val.len <- length(w.vals)
delta=pmin(0.08*w.vals,rep(0.01,w.val.len))


CDF.T0<-array(0,dim=c(w.val.len,3))
CDF.TA<-array(0,dim=c(w.val.len,3))
ecdf.T0<-list()
...
#w.vals<-c(0.45,0.9)
min.config.stress.1.w<- rep(0, w.val.len)
min.config.stress.2.w<- rep(0, w.val.len)
...
T0.1.delta <-array(0, dim= c(length(grid.seq.x),length(grid.seq.y),oos.copies))
TA.1.delta <-array(0, dim= c(length(grid.seq.x),length(grid.seq.y),oos.copies))
T0.2.delta <-array(0, dim= c(length(grid.seq.x),length(grid.seq.y),oos.copies))
TA.2.delta <-array(0, dim= c(length(grid.seq.x),length(grid.seq.y),oos.copies))


T0.w.1 <-array(0, dim= c(w.val.len,length(grid.seq.x)*length(grid.seq.y)*oos.copies))
TA.w.1 <-array(0, dim= c(w.val.len,length(grid.seq.x)*length(grid.seq.y)*oos.copies))
T0.w.2 <-array(0, dim= c(w.val.len,length(grid.seq.x)*length(grid.seq.y)*oos.copies))
TA.w.2 <-array(0, dim= c(w.val.len,length(grid.seq.x)*length(grid.seq.y)*oos.copies))
T0.w.1.delta <-array(0, dim= c(w.val.len,length(grid.seq.x)*length(grid.seq.y)*oos.copies))
TA.w.1.delta <-array(0, dim= c(w.val.len,length(grid.seq.x)*length(grid.seq.y)*oos.copies))
T0.w.2.delta <-array(0, dim= c(w.val.len,length(grid.seq.x)*length(grid.seq.y)*oos.copies))
TA.w.2.delta <-array(0, dim= c(w.val.len,length(grid.seq.x)*length(grid.seq.y)*oos.copies))




for (w.i in 1:w.val.len){
  for (delta.add in c(-1,1)){
    w <- w.vals[w.i]+(delta.add*delta[w.i])
    print("w value is")
    print(w)
    ...
    for (j in 1:length(grid.seq.y)){
      ...
      for (i in 1:length(grid.seq.x)){
        ...
        if (delta.add==-1){
          ...
          T0.1.delta[i,j,]<- sqrt(rowSums((X.embed.2.norm[indices.test.cluster.5,]-matrix(X.embed.2.norm[2,],oos.copies,2,byrow=TRUE))^2))
          T0.2.delta[i,j,]<- sqrt(rowSums((X.embed.2.norm[indices.test.cluster.6,]-matrix(X.embed.2.norm[4,],oos.copies,2,byrow=TRUE))^2))
          TA.1.delta[i,j,]<- sqrt(rowSums((X.embed.2.norm[indices.test.cluster.5,]-matrix(X.embed.2.norm[4,],oos.copies,2,byrow=TRUE))^2))
          TA.2.delta[i,j,]<- sqrt(rowSums((X.embed.2.norm[indices.test.cluster.6,]-matrix(X.embed.2.norm[2,],oos.copies,2,byrow=TRUE))^2))
    ...
    if (delta.add==-1){
      ...
      T0.w.1.delta[w.i,]<-as.vector(T0.1.delta)
      T0.w.2.delta[w.i,]<-as.vector(T0.2.delta)
      TA.w.1.delta[w.i,]<-as.vector(TA.1.delta)
      TA.w.2.delta[w.i,]<-as.vector(TA.2.delta)
...
for (w.i in 1:w.val.len){
  ...
  ecdf.T0.delta <- c(ecdf.T0.delta,list(ecdf(T0.w.1.delta[w.i,])))
  ...
  ecdf.TA.delta <- c(ecdf.TA.delta,list(ecdf(TA.w.1.delta[w.i,])))
}
...
for (w.i in 1:w.val.len){
  ...
  par(mfrow=c(2,1))
  par(lty=1)
  plot(ecdf.T0[[w.i]],main="top:T0,T0.delta bottom:TA,TA.delta ")
  par(lty=3)
  lines(ecdf.T0.delta[[w.i]],main="")
par(lty=1)
plot(ecdf.TA[[w.i]],main="")
par(lty=3)
lines(ecdf.TA.delta[[w.i]],main="")
...
bw.type <- 0.05
if (run.in.linux) {X11()} else {windows()}
#par(mfrow=c(w.val.len/2,2))
for (w.i in 1:w.val.len){
  h.1<-density(T0.w.1[w.i,],bw=bw.type)
  h.2<-density(T0.w.1.delta[w.i,],bw=bw.type)
...
for (w.i in 1:w.val.len){
  ...
  bw.type <- h.1$bw
  par(lty=1)
  if (w.i==1){ plot(h.1,type="l",col=colors.vec[w.i],main="")
  }else {lines(h.1,main="",col=colors.vec[w.i],main="")}
  par(lty=3)
  lines(h.2,main="",col=colors.vec[w.i])
  
  title("Multiple w and w+delta values in diff colors")
}
legend.txt<-w.vals
legend("bottomright", legend=legend.txt, col=colors.vec, lty=rep(1, w.val.len))
...
for (w.i in 1:w.val.len){
  ...
  h.2<-density(T0.w.2.delta[w.i,],bw=bw.type)
  
  bw.type <- h.1$bw
  par(lty=1)
  if (w.i==1){plot(h.1,xlim=c(0,3.5),ylim=c(0,0.8))
  }else {lines(h.1,xlim=c(0,3.5),ylim=c(0,0.8))}
  par(lty=3)
  lines(h.2,main="T0.w.2 and T0.w.2.delta (NUll for w equal to w-delta and w+delta)")
}
...
for (w.i in 1:w.val.len){
  ...
  h.2<-density(TA.w.1.delta[w.i,],bw=bw.type)
  bw.type <- h.1$bw
  par(lty=1)
  if (w.i==1){ plot(h.1,xlim=c(0,3.5),ylim=c(0,0.8))
  }else {lines(h.1,xlim=c(0,3.5),ylim=c(0,0.8))}
  par(lty=3)
  lines(h.2,main="TA.w.1 and TA.w.1.delta (alternative for w equal to w-delta and w+delta)")
}
...
for (w.i in 1:w.val.len){
  ...
  h.2<-density(TA.w.2.delta[w.i,],bw=bw.type)
  bw.type <- h.1$bw
  par(lty=1)
  if (w.i==1){ plot(h.1,xlim=c(0,3.5),ylim=c(0,0.8))
  }else {lines(h.1,xlim=c(0,3.5),ylim=c(0,0.8))}
  par(lty=3)
  lines(h.2,main="TA.w.2 and TA.w.2.delta (alternative for w equal to w-delta and w+delta)")
}
