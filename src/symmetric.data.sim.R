...
agg.test.stats.null.JOFC <- NULL
agg.test.stats.alt.JOFC <- NULL
...
#		 }
	...
	i<-1
			...
			X1<-X1.X2[[1]]$X1
			X2<-X1.X2[[1]]$X2
			for (j in 2:train.reps){
				X1<-rbind(X1,X1.X2[[j]]$X1)
				X2<-rbind(X2,X1.X2[[j]]$X2)
			}
			
			
			
#	alpha.matched<-rdirichlet(rep(1,4))
			#num.test.pts  pairs based on the same point coord.matrix[i,]
			# The statistic computed according to  this generation mechanism is not 
			# really the distribution under null, since all of the pairs are based on a single point
			#  This is done to study fidelity and commensurability phenomenon for simplex example 
			alpha.matched<-coord.matrix[i,]
			for (j in 2:num.test.pts)
				alpha.matched <- rbind(alpha.matched,coord.matrix[i,])
			test.dirich.data<- matched_rdirichlet(n=num.test.pts, p=simplex.dim-1,
					r=r, q=3, c=0, alpha=alpha.matched)
			
			
			##num.test.pts  pairs  for a random point in simplex
			alpha.unmatched<-rdirichlet(num.test.pts,rep(1,simplex.dim))
			
			for (j in 2:num.test.pts)
				alpha.unmatched<-rbind(alpha.unmatched,rdirichlet(num.test.pts,rep(1,simplex.dim)))
			...
			## ==== jofc ====
			if (oos){
				JOFC.stats<- run.jofc(D1,D2,D10,D20,D2A,D.oos.1,D.oos.2.null,D.oos.2.alt, ideal.omnibus.0,ideal.omnibus.A,
...
#		   Y2At<- X[alt.pt.index.X2,]
			...
			T0 <- JOFC.stats$T0
			TA <- JOFC.stats$TA
			if (is.null(agg.test.stats.null.JOFC)){
				agg.test.stats.null.JOFC <- T0
				agg.test.stats.alt.JOFC  <- TA
				
			} else {
				agg.test.stats.null.JOFC <- cbind(agg.test.stats.null.JOFC,T0)
				agg.test.stats.alt.JOFC  <- cbind(agg.test.stats.alt.JOFC ,TA)
			}
		}
	#}
...
hist(agg.test.stats.alt,breaks=h.breaks,add=TRUE,col="blue")
hist(agg.test.stats.null.hub.pt,breaks=h.breaks,add=TRUE,col="purple")
...
power.vals<-matrix(0,num.w.vals,101)
for (w.val.l in 1:num.w.vals){
	windows()
	plot( density(agg.test.stats.null.JOFC[w.val.l,]),
			xlim=range( c(agg.test.stats.null.JOFC[w.val.l,],agg.test.stats.alt.JOFC[w.val.l,]
					) ),
			col="red",main="JOFC stat null.vs.alt", xlab="" )
	lines(density(agg.test.stats.alt.JOFC[w.val.l,]), col="blue")
	
	power.vals[w.val.l,]<-get_power(agg.test.stats.null.JOFC[w.val.l,],agg.test.stats.alt.JOFC[w.val.l,],seq(0,1,0.01))
...
cols.vec<-rainbow(num.w.vals)

windows()
plot(power.vals[1,],type="l")
for (j in 2:num.w.vals)
lines(power.vals[j,],col=cols.vec[j])
