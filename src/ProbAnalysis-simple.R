...
colors.vec<-c("red","blue","green","black","gold4","azure","darkgrey","purple")

par(lwd=2)
for (i in 1:w.max.index){
  windows()
   par(lty=1)
  plot(density(oos.diss.all.params.0[1,i,],bw=0.25), col= colors.vec[1],lty=1,type='l',ylim=c(0,0.2))
  for (j in 2:4){
    lines(density(oos.diss.all.params.0[j,i,]), col= colors.vec[j])
  }

  par(lty=3)
  lines(density(oos.diss.all.params.A[1,i,],bw=0.25), col= colors.vec[1],lty=3,ylim=c(0,0.2))
  for (j in 2:4){
    lines(density(oos.diss.all.params.A[j,i,]), col= colors.vec[j])
  }
  windows()
   par(lty=1)
  plot(density(oos.diss.all.params.0[1,i,],bw=0.25), col= colors.vec[1],lty=1,type='l',ylim=c(0,0.4))
  lines(density(oos.diss.all.params.0[5,i,]), col= colors.vec[5])
   par(lty=3)
  lines(density(oos.diss.all.params.A[1,i,],bw=0.25), col= colors.vec[1])
  lines(density(oos.diss.all.params.A[5,i,]), col= colors.vec[5])
}
