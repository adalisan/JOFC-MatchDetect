...
\end{comment}
\subsubsection{ A short detour : Discontinuity in configurations}
...
The alternative was chosen as $ H_{A1}$ and SUMCOR was chosen for generalized CCA.
 $q$-dimensional noise vectors of magnitude $c$ was added to the matched measurements. 
 The ROC curves are shown in \ref{fig:MVN-c001-ROC-Kcond}.
...
<<FidComm-d-plot-Gaussian, echo=FALSE,fig=FALSE,results=hide,cache=TRUE >>=
w.val.len<-length(w.vals)
...
<<FidComm-d-plot-Dirichlet, echo=FALSE,fig=FALSE,results=hide,cache=TRUE >>=
...
if (verbose) print(c("Fid1","Fid2","Comm"))
if (verbose) print(cbind(Fid1.D,Fid2.D,Comm.D))
if( run.in.linux) X11() else {windows()}
...
for (w.i in 1:w.val.len){
	if (w.i==1) {plot (d.vals,avg.Fid1.alt.D[first.ind, w.i],type="l",col="red")}
	else {lines (d.vals,avg.Fid1.alt.D[first.ind, w.i],col="red")}
	lines(d.vals,avg.Fid2.alt.D[first.ind, w.i],col="orange")
	lines(d.vals,avg.Comm.alt.D[first.ind, w.i],col="blue")
}



for (w.i in 1:w.val.len){
	if (w.i==1) {plot (d.vals,avg.FCratios.List.D[first.ind,w.i],type="l",col="red")}
	else {lines (d.vals,avg.FCratios.List.D[first.ind,w.i],col="red")}
	lines(d.vals,avg.wtFCratios.List.D[first.ind,w.i],col="blue")
...
The next step is to use the pairwise distances to find the optimal 1-1 matchings by the Hungarian algorithm \cite{Hung-algo}.
...
In our experiments we will use three different dissimilarities:
\begin{itemize}
 ...
 \end{itemize}
 Note that these dissimilarities can only be defined between vertices of the same graph. We impute the inter-condition dissimilarities as
  as described before\label{omnibus}.
 
  In the following simulation, $A$ is an Erdos-Renyi graph and the adjacency matrix of $B$ is a entry-wise bit-flipped version of the adjacency matrix of $A$.
  The probability of flipping an entry of the adjacency matrix is the perturbation parameter which is the variable on the x-axis. 
  The performance measure is the proportion of true measures.
...
<<JOFC-graph-sim-2, echo=FALSE,fig=FALSE,results=hide,cache=TRUE >>=
