...
<<sim-two-cond-pre,fig=FALSE,echo=FALSE,results=hide,cache=TRUE,prefix.string="JOFCpaper",trace=TRUE>>=
rm(list=ls())
graphics.off()
...
<<sim-Kcond-pre,fig=FALSE,echo=FALSE,results=verbatim,cache=TRUE,prefix.string="JOFCpaper">>=
#rm(list=ls())
...
<<wiki-2cond-plot, echo=FALSE,fig=TRUE,results=hide,cache=FALSE,dependson=wiki-2cond >>=
...
params$w.vals = c(0.5,0.8,0.925,0.99,0.999)
...
#<<FidComm-d-plot-Gaussian,eval=FALSE, echo=FALSE,fig=FALSE,results=verbatim,cache=TRUE,dependson=FidComm-d-sim >>=
run.for.Sweave<-TRUE
...
plot.graph.with.CI(Fid1,"Fid and Comm Terms","red",conf.int=TRUE,   add=FALSE,fp.points=params$w.vals,
		customx.labels=NULL,customy.labels=NULL,ispowercurve=FALSE)

plot.graph.with.CI(Fid2,"Fid and Comm Terms","orange",conf.int=TRUE,add=TRUE,fp.points=params$w.vals,
		customx.labels=NULL,customy.labels=NULL,ispowercurve=FALSE)

plot.graph.with.CI(Comm,"Fid and Comm Terms","blue",conf.int=TRUE,  add=TRUE,fp.points=params$w.vals,
		customx.labels=NULL,customy.labels=NULL,ispowercurve=FALSE)
...
for (i in 1:d.vals.len)
	hist(best.w.g.list.param.i,breaks=length(params$w.vals),
