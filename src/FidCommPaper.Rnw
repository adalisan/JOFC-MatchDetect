...
\graphicspath{{./}{./../graphs/}{./../../DataFusion/graphs/}{./../../DataFusion_Kconditions/graphs/}}
...
\SweaveOpts{echo=FALSE,results=hide,debug=TRUE}
<<>>=
	rm(list=ls())
...
\subsection{ A short detour : Discontinuity in configurations}
...
\end{comment}
Note that it is possible to have multiple local minima in the embedding step(see example in$\ref{}$) and have $w$ control which is the global minimum among the configurations of $\hat{X}_{.}$. 
Global Minimum Configuration determined by $w$ .

Consider four in-sample points in $\mathbb{R}$ with locations $X_1=(0,0)$,$X_2=(1,0)$,$X_3=(1,1)$ and $X_4=(1,0)$ and two out-of-sample  points with coordinates $X_5=(0.95,1)$ and $X_6=(0.07,1)$. Suppose $X_5$ and $X_3$ are matched. Denote the Euclidean distance matrix by $D$. Suppose, due to noise, or due to dissimilarities not being Euclidean distances, our dissimilarity matrix is $D'=D-0.92$. Based on the initial configuration, the  embedding coordinates of $\hat{X}_5$ might be closer to $X_4$ compared to $X_3$. This is due to a local minimum in the configuration space, because while $\hat{X}_5$ starts from the initial configuration, it might have to cross paths with the embedding of  ${X}_4$ with which it has a "large`` dissimilarity.  Based on value of $w$, it might be easier to get out of this local minimum. In fact, depending on w can this  local minimum can be a global minimum. The following plot shows what minimum $\hat{X}_5$ ends up in, depending on $w$ value.   

Other than such carefully constructed examples, we expect the configuration that is a minimum to be unique.  
 Another avenue for investigation is  how $w^{*}$ depends on the parameters of the distributions of $\mathbf{F}_m$ and  $\mathbf{F}_u$ . 
 For example consider the signal and noise dimensions $p$ and $q$ respectively.
...
<<fig-stats-p, echo=FALSE,fig=TRUE,results=hide,cache=FALSE>>=
  fix.i <-1
   ...
   par(lty=1)
  plot(density(oos.diss.all.params.0[1,fix.i,],bw=0.25), col= colors.vec[1],
		  lty=1,type='l',ylim=c(0,0.2))
  for (j in 2:4){
    lines(density(oos.diss.all.params.0[j,fix.i,],bw=0.25), col= colors.vec[j])
  }

  par(lty=3)
  lines(density(oos.diss.all.params.A[1,fix.i,],bw=0.25), col= colors.vec[1],lty=3,ylim=c(0,0.2))
  for (j in 2:4){
    lines(density(oos.diss.all.params.A[j,fix.i,],bw=0.25), col= colors.vec[j])
  }
@
...
Consider increase in $r$, this will cause the test statistic under null to be stochastically smaller,
 ...
 Consider increase in $r$, this will cause the test statistic under null to be stochastically smaller,
  leading to a smaller critical value. So , a increase in fidelity, which corresponds to smaller $w$ might lead to the increase in the test statistic under alternative, 
  and therefore an increase in power.  
 Consider increase in $r$, this will cause the test statistic under null to be stochastically smaller,
  leading to a smaller critical value. So , a increase in fidelity, which corresponds to smaller $w$ might lead to the increase in the test statistic under alternative, 
  and therefore an increase in power.  
...
par(lty=1)
	plot(density(oos.diss.all.params.0[1,fix.i,],bw=0.25), col= colors.vec[1],
		  lty=1,type='l', ylim=c(0,0.4))
 	for (j in 2:4){
		lines(density(oos.diss.all.params.0[1+(j-1)*5,fix.i,],bw=0.25), col= colors.vec[j])
  	}
	
	par(lty=3)
	lines(density(oos.diss.all.params.A[1,fix.i,],bw=0.25), col= colors.vec[1])
	for (j in 2:4){
		lines(density(oos.diss.all.params.A[1+(j-1)*5,fix.i,],bw=0.25), col= colors.vec[j])
  	}
@
...
<<stats-q-sim, echo=FALSE,fig=TRUE,results=verbatim,cache=TRUE >>=
source("./src/Null.Alt.Stat.for.JOFC.R")
...
fix.i<-1
	...
	par(lty=1)
  plot(density(oos.diss.all.params.q.0[1,fix.i,],bw=0.25), col= colors.vec[1],
		  lty=1,type='l',ylim=c(0,0.4))
 for (j in 2:length(q.vals)) {
  lines(density(oos.diss.all.params.q.0[j,fix.i,],bw=0.25), col= colors.vec[j])
	}
   par(lty=3)
  lines(density(oos.diss.all.params.q.A[1,fix.i,],bw=0.25), col= colors.vec[1])
  for (j in 2:length(q.vals)) {
  lines(density(oos.diss.all.params.q.A[j,fix.i,],bw=0.25), col= colors.vec[j])
  }
@
...
@
<<fig=FALSE,echo=FALSE,results=verbatim,cache=TRUE,prefix.string="JOFCpaper">>=
print(debug.mode)
@

<<sim-two-cond,fig=FALSE,echo=FALSE,results=hide,cache=TRUE,prefix.string="JOFCpaper">>=
debug.mode<- FALSE
...
\begin{figure}
\includegraphics[scale=0.35]{MVN-FC-Tradeoff-OOSc0-01.pdf}
...
\begin{figure}
\includegraphics[scale=0.35]{Dirichlet-FC-Tradeoff-OOSc0.pdf}
\caption{Power ($\beta$) vs Type I error ($\alpha$) plot for different $w$ values for the Gaussian setting (noisy case)}
\caption{$\beta$ vs $\alpha$ plot for different $w$ values for the Dirichlet setting (noiseless case)}
\label{fig:Dir-c001-power-alpha}
\end{figure}

\begin{figure}
\includegraphics[scale=0.35]{Dirichlet-FC-Tradeoff-OOSc0-01.pdf}
\caption{$\beta$ vs $\alpha$ plot for different $w$ values for the Dirichlet setting (noisy case)}
\label{fig:Dir-c001-ROC}
\end{figure}
...
As it was mentioned, all of the approaches are generalizable to $K>2$ conditions, though some ambiguities need to be resolved. For example,  the alternative hypothesis could be  defined as the event that at least one of the K new dissimilarities are unmatched ($ H_{A1}: \exists i, j , 1\leq i < j \leq K :\bm{y}_{i} \nsim \bm{y}_{j} $ ) or it could be defined as the case that absolutely none of the K dissimilarities are matched   ($H_{A2}: \forall i, j , 1\leq i < j \leq K :\bm{y}_{i} \nsim \bm{y}_{j}$ )  . and the sample from the alternative must be generated accordingly during Monte Carlo simulation. In the first case, an appropriate test statistic is then the maximum of the pairwise distances between embeddings of each test K-tuple.

For PoM, one can use Procrustes analysis  generalized to more than two configurations.

For CCA, there are multiple generalizations available as the correlation between more than two configurations can be defined in multiple ways \cite{generalCCA}. Let $X_1,\ldots,X_k$ be random vectors. Consider the selection of the first set of directions to be selected for CCA and the correlation matrix of   $X_1,\ldots, X_k$, $\Phi_1$.   The following two criteria  are proposed in \cite{generalCCA}.
\begin{itemize}
\item MAXVAR. Maximize the largest eigenvalue of $\Phi_1$ : $\lambda^{(1)}_1$ 
\item  MINVAR. Minimize  the smallest eigenvalue of $\Phi_1$ : $\lambda^{(m)}_1$ 
\end{itemize}
For $H_{A1}$ MINVAR might be a good choice, since the exploitation task is to find evidence against the null and as favorable to the alternative as possible. MINVAR should be small whenever there exists \emph{at least} one unmatched measurement so under the alternative hypothesis. MAXVAR, in this case, wouldn't be necessarily small under alternative with respect to null.  MINVAR statistic should be stochastically smaller compared to null distribution. For $H_{A2}$ MAXVAR is appropriate, since under the alternative hypothesis, MAXVAR should be very small compared to under the null hypothesis. 
...
A collection of articles are collected from the English Wikipedia, consisting of the directed 2-neighborhood of the document "Algebraic Geometry"
   This  collection of 1382 articles and the correspondence of each article in French 
Wikipedia forms our real-life dataset. The dissimilarity between articles in same language is computed by the Lin  Pantel discounted mutual information \cite{} and cosine dissimilarity k(xik; xjk) = 1  (xik xjk)=(kxikk2kxjkk2).  
For hypothesis testing,   randomly held out four documents - one matched pair and one unmatched pair
 -  are used to compute empirical type I error $\alpha$ and estimate of power based on the critical value computed from the distribution of the test statistic for the remaining 1380 matched pairs. 
The test statistic is computed using one of the three approached mentioned  $cca$, $p\circ m$, and $jofc$ . The two sets of held-out matched pairs are used as y1 and y2, via out-of-sample
embedding, to estimate the null distribution of the test statistic T = d(ye1; ye2). This allows
us to estimate critical values for any specified Type I error level. Then the two sets of heldout unmatched pairs are used as y1 and y2, via out-of-sample embedding, to estimate power.
Target dimensionality d is determined by the Zhu and Ghodsi automatic dimensionality selection
method [26], resulting in d = 6 for this data set.
