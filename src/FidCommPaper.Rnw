...
Consider five in-sample points in $\mathbb{R}$ with locations $X_1=(0,0)$,$X_2=(1,0)$,$X_3=(1,1)$
 and $X_4=(1,0)$, $X_5=(.5,.5)$ and two out-of-sample  points with coordinates $X_6=(1,0)$ and $X_7=(0,1)$.
  ...
  D_ij-1.4 & \textrm{if  $i==4 & j==6$ or $i==6 & j==4$ or $i==2 & j==7$  or $i==7 & j==2$}  \\
  D_ij  & \textrm{ otherwise}\\
  \end{cases}$. Quantitatively, the three points $X_1$, $X_7$ and $X_3$ form a barrier which to cross for the oos points to reach their matched counterparts.
   
   Based on the initial configuration, the  embedding coordinates of $\hat{X}_6$ might be closer
    to $X_4$ compared to $X_2$. This is due to a local minimum in the configuration space, 
    because while $\hat{X}_6$ starts from the initial configuration at the start of optimization, 
...
For PoM, one can use Procrustes analysis  generalized to more than two configurations. For the development of Generalized Procrustes Analysis, see \cite{generalPCA} 
...
\item  MINVAR. Minimize  the smallest eigenvalue of $\Phi_1$ : $\lambda^{(m)}_1$ 
\end{itemize}
For $H_{A1}$ MINVAR might be a good choice, since the exploitation task is to find evidence against the null and as favorable to the alternative as possible. MINVAR should be small whenever there exists \emph{at least} one unmatched measurement so under the alternative hypothesis. MAXVAR, in this case, wouldn't be necessarily small under alternative with respect to null.  MINVAR statistic should be stochastically smaller compared to null distribution. For $H_{A2}$ MAXVAR is appropriate, since under the alternative hypothesis, MAXVAR should be very small compared to under the null hypothesis. It is quite possible the other generalizations are appropriate for our task 
...
A collection of articles are collected from the English Wikipedia, consisting of the
...
Wikipedia is the real-life dataset. These documents are processed, and dissimilarities between documents  in same language are computed by the Lin  Pantel discounted mutual information \cite{LinPantel}
 and cosine dissimilarity $k(x_{ik}; x_{jk}) = 1  (x_{ik} x_{jk})=(k(x_{ik})k2kx_{jk}k2)$. It is possible to utilize both textual content of the documents and the hyperlink graph structure.
...
We wish to apply our approach to  the vertex matching problem in multiple graphs. We are again in a semi-supervised setting, where matchings between some vertices in different graphs are known  and wish to infer the correspondences between the remaining collection of vertices in the graphs.
\begin{comment}
a formal description of the problem with notation
\end{comment} 
One useful property of dissimilarity representation is that the structure of data is irrelevant once an appropriate dissimilarity function  for the data is available. There are many distances that can be defined between vertices in graphs. We assume that an appropriate distance measure is available to us.
