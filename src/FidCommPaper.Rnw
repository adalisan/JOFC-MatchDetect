...
<<FidComm-d-plot-Dirichlet, echo=FALSE,fig=FALSE,results=hide,cache=FALSE >>=
...
<<FidComm-d-plot-Dirichlet-2, echo=FALSE,fig=TRUE,results=hide,cache=FALSE >>=
if( run.in.linux) X11() else {windows()}
...
<<FidComm-d-plot-Dirichlet-3, echo=FALSE,fig=TRUE,results=hide,cache=FALSE >>=
for (w.i in 1:w.val.len){
...
\begin{itemize}
 ...
 Note that these dissimilarities can only be defined between vertices of the same graph. We impute the inter-condition dissimilarities as
  ...
  The probability of flipping an entry of the adjacency matrix is the perturbation parameter $p$ which is the variable on the x-axis. 
