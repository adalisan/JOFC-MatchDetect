...
\begin{comment}
To emphasize the fact that we use the simple weighting scheme with $w$, we will rewrite the criterion function as  $\sigma_w(\widetilde{X},\Delta^{(.)})$ for JOFC embedding. 
\end{comment}
...
\begin{comment}
We further make the reasonable assumption that $T_a(w)$ is stochastically larger $T_0(w)$ for $w \in (0,1)$. Then, the continuity of $AUC(w)$ with respect to $w$ in $(0,1)$, along with the behaviour of $AUC(w)$ in the neighborhood of $w=0$ and $w=1$, is sufficient to guarentee  that a global maximum $w^*$ exists. As we have proved the continuity of the function, let us describe the behaviour of $AUC(w)$ in the neighborhoods of these endpoints. 

Consider the limits of the AUC function as $w\rightarrow 0$  and as $w\rightarrow 1$. As $w\rightarrow 0$, the distribution of $T_0(w)$ and $T_a(w)$ will be increasingly similar, since  minimization of commensurability errors does not have high priority. In fact, $AUC(w)$  would converge to the AUC of a Procrustes$\circ$MDS procedure (an embedding followed by Procrustes Analysis on the embeddings). As $w\rightarrow 1$, minimizing  the commensurability error becomes extremely important, and both  $T_0(w)$ and $T_a(w)$ becomes extremely peaked around $0$. When $w=1$, both $T_0(w)$ and $T_a(w)$ is 0 with probability 1.
 So as $w$ goes towards either of  the endpoints of  the interval $(0,1)$, $AUC(w)$ is decreasing. Therefore, even if the AUC function has discontinuities at  the endpoints, $AUC(w)$ will attain its global maximum in the interval $(0,1)$.
\end{comment}
...
\begin{comment}
Furthermore, we can make an argument for the unimodality of $AUC(w)$ and uniqueness of $w^*$. We have already mentioned the behaviour of $AUC(w)$ in the neighborhoods of the endpoints. With regards to the AUC function, we are interested in the probability  $P(T_a(w)>T_0(w))$ and the probability ``$T_0(w)$  is smaller than an arbitrary threshold''. As $w$ goes from 0 towards any argmax of $AUC$, the  probability ``$T_0(w)$  is small'' increases,  while the probability that ``$T_a(w)$ is just as small'' can not be very large. As $w$ goes from 1 to any argmax of $AUC$, the  probability ``$T_0(w)$  is small'' does not change  much, while the  probability ``$T_a(w)$ is just as small'' should decrease. Assuming the probability distributions of $T_0(w)$ and $T_a(w)$ have this general behaviour everywhere in $ (0,1)$, there should be a single $w$ value at which any infinitesimal increase (decrease) in the probability ``$T_0(w)$  is small'', will be offset by the probability increase (decrease) in  ``$T_a(w)$ is just as small'' (therefore resulting in no increase in the probability $P(T_a(w)>T_0(w))$.
...
\begin{figure}[tb]
     \centering
\includegraphics[scale=0.3,angle=0]{plot_zoom.png}
...
\begin{figure}[tb]
  %\captionsetup[subfigure]{subrefformat=parens,labelformat=parens}
      \centering
         \includegraphics[scale=0.35]{OOSMVN-power-w-c0-paper-final.pdf}
...
\begin{figure}[h]
	\centering
		...
		\includegraphics[scale=0.15]{auc_errorbar_plot.pdf}
