...
An interesting question that will not be addressed in this work is whether any of these generalizations is more appropriate for $H_{A1}$ or $H_{A2}$.
...
The alternative was chosen as $ H_{A1}$ and SUMCOR criterion was chosen as  generalized  CCA. The ``Noisy" case will be investigated, i.e. 
...
Our exploitation task is still testing for matchedness of vertices between different conditions, in this case wiki articles that are on the same topic  in different languages.
...
\includegraphics{FidCommPaper-graph-plot-5}
