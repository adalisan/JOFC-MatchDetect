...
\subsection{Continuity of $\beta(\cdot)$} 
 Consider $\beta_{\alpha}(\cdot)$ as a function of $w$. We can write it as $Pr\left[T_a(\cdot)>c_{\alpha}(\cdot)\right]$ where $c_{\alpha}(\cdot)$ is the critical value for level $\alpha$. To prove the continuity of $\beta_{\alpha}(\cdot)$
...
\begin{thm}
Let $T(\cdot)$ be  a stochastic process indexed by $w$ in the interval (0,1). Assume  the process is continuous in probability  (stochastic continuity)   everywhere in the interval , ie.
$$ \forall a>0 \quad  \lim_{\delta \rightarrow 0} Pr\left[\|T(w+\delta)-T(w) \|>a \right] \rightarrow 0$$ 
...
\begin{proof}
We know there exists a $\delta$ for any given $\epsilon>0$. For any such $\delta$, consider the difference of the probabilities of the two events  $T(w)>0$ and $T(w+\delta)>0$ for any $w$.
\begin{eqnarray*}
...
& <= & \| Pr\left[\{T(w+\delta) - T(w)  > 0\} \vee \{T(w)- T(w+\delta)  > 0 \}  \right]\|\\
...
\begin{cor}{
. Then $AUC(w)=P\left[T_a(w)-T_0(w) >0 \right]$ is continuous with respect to $w$.}
\end{cor}
\begin{proof}
