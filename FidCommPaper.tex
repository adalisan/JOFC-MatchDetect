...
\begin{itemize}
 ...
 \item weighted extension Czekanowski-Dice distance \cite{DICE,weightedDICE}
 \end{itemize}
...
There are a lot  of interesting questions to ponder about the number of known correspondences, such as , how many known correspondences are necessary for satisfactory performance for graphs of some fixed size and whether , in the ``match ratio'' vs number of known correspondences curve,  there are any ``elbows'' , after which the cost of more correspondences are not justified by the accompanying increase in ``match ratio''. Figure xxx shows ``match ratio'' plotted against number of ``seeds'' for the same bitflip experiments.
