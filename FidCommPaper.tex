...
\end{eqnarray*} 
%\end{center}
For any $w$, choose $\delta$ such that such that both $ Pr\left[T(w+\delta)>0   | T(w) \leq 0 \right] <\epsilon$ and  $Pr\left[T(w+\delta)\leq 0  | T(w) > 0\right]<\epsilon $. Such a $\delta$ exists due to stochastic continuity. Therefore 
\[
