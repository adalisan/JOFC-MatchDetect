...
\begin{comment}
One can approach this problem in a geometric way in which points are assumed to lie on some low-dimensional manifold up to some error. Or one could use a probabilistic approach where the distribution of data is assumed to have a support that is mostly concentrated on a few dimensions of the measurement space. 
While the representations in the low-dimensional space may be assumed to be common to all of the views, we have  relaxed  this assumption in our approach that they are mostly commensurate, but there is some incommensurability error. 
\end{comment}
...
\subsection{Continuity of $\beta(w)$} 
   Consider $\beta(w)$ as a stochastic process whose sample path is  a function of $w$ where the randomness comes from $\Delta_{m}$, $\Delta_{u}$ and $\mathcal{T}$.  As a surrogate to $\beta(w)$, the area under the curve measure will be shown to be continuous: $AUC(w)=P\left[T_a(w)>T_0(w)\right]$ where $T_a(w)$ and $T_0(w)$ can also be regarded as  stochastic processes whose sample paths are continuous functions of $w$ except finite number of points in $(0,1)$. By  assuming stochastic continuity of $T_a(w)$ and $T_0(w)$ in the interval $(0,1)$, ie assuming the probability of a jump discontinuity of $T_0(w)$ at a particular $w_0$ is 0, we will prove the continuity of $AUC(w)$.
...
Setting p and q to 5 and 10, respectively, for $n=150$ and $m=150$, the average of the power values for $nmc=150$ Monte Carlo replicates are computed at  different $\alpha$s and are plotted in Figure \ref{fig:MVN-c001-ROC} against $\alpha$ for the Gaussian setting.  Qualititatively similar plots for the Dirichlet setting  are not included for brevity.  The plot in Figure \ref{fig:MVN-c001-ROC} shows that for different values of  $w$, $\beta$-$\alpha$ curves vary significantly.  The match detection tests with JOFC embedding with specific $w$ values have better perform others in terms of power. In Figure \ref{fig:MVN-c001-beta-w},  $\beta(w)$ is plotted against $w$ for fixed values of $\alpha$. It is  interesting that the optimal value of $w$ seems to be in the range of $(0.85,1)$ for all settings, which suggests  a significant emphasis on commensurability might be  critical for our hypothesis testing task. 
...
Under the null  hypothesis,
 $Pr[\left(c_1c_2\right)=(TF)]=Pr[\left(c_1c_2\right)=(FT)]$, so $\sum_l{I \{e_{TF}^{(l)}>e_{FT}^{(l)}\}}$ will be distributed according to  the binomial distribution, $\mathcal{B}(nmc,0.5)$. ($I\{\cdot\}$ is the indicator function.) 
...
setwd("./DataFusion/")
K<-3
sink("./logs/debug.trymsgKcond.txt")
