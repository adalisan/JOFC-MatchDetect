...
\SweaveOpts{echo=FALSE,results=hide,debug=TRUE,prefix.string=graphs/FidCommPaper}



<<Init,cache=FALSE,results=hide>>=
	rm(list=ls())
	#setwd(file.path("./../"))
...
An closed-form expression for this function will be hard to find  even in the most simple of cases. So consider the estimate from the Monte Carlo simulation $\beta^{(m)}_{\alpha}(w)$ for the estimate computed from $2m$ testing pairs, m matched pairs and m unmatched pairs. By LLN $\beta^{(m)}_{\alpha}\rightarrow\beta_{\alpha}(w)$ pointwise. We will assume an arbitrary element $\omega$ of sample space $\Omega$. Suppose the test statistic values  are $T_0^{i}$, $T_A^{i}$ $i=1,\ldots,m$  respectively for matched and unmatched test pairs.Note that $\beta^{(m)}_{\alpha}$ can be written as $\sum_{j=1}^m I(T_A^{j}>T_0^{(i)})$ where $T_0^{(i)}$ is the $i=\lceil {m*(1-\alpha)} \rceil^{th}$ order statistic of $T_0^{i},i=1,\ldots,m$. Replacing the indicator function with the unit step function
\[
\sum_{j=1}^m u(T_A^{j}-T_0^{(i)})
\]
...
To prove differentiability of $\beta_{\alpha}\left( w\right)$ , we refer to Theorem 5.3.3 in \cite{ElClassAnalysis-5-3-3}, which requires that $\mathcal{B}_{alpha}^{(m)}$ converges pointwise to  $\beta_{\alpha}\left( w\right)$ , that $\frac{d\mathcal{B}_{alpha}^{(m)}}{dw}$ are continuous and that they  converge \emph{uniformly} to a function $g$. Since the sigmoid function $\sigma(\cdot)$ is continously differentiable, our attention will be on $ \frac{\partial{\T_0^{j}}}{\partial{w}}$ and $ \frac{\partial{\T_0^{j}}}{\partial{w}}$. We show  $\T_0^{j}$ and $\T_A^{i}$ to be not  continuous everywhere for a particular example in \ref{subsubsec:Discontinuity}. 
...
<<multMin,  echo=FALSE,fig=FALSE,results=hide,cache=TRUE ,trace=TRUE>>=
...
<<multMin-table,  echo=FALSE,fig=FALSE,results=tex ,dependson=multMin>>=
...
Does the discontinuity in configurations mean discontinuity  in the $\beta(w)$ function. Consider $\beta(w)$ as a stochastic process whose sample path is  a function of $w$ where the randomness comes from $\Delta_{m}$, $\Delta_{u}$ and $\mathcal{T}$ .  As a surrogate, we will make use of the area under the curve measure: $AUC(w)=P\left[T_a(w)>T_0(w)\right]$ where $T_a(w)$ and $T_0(w)$ can also be regarded as  stochastic processes whose sample paths are continuous functions of $w$ except finite number of points in $(0,1)$. By  assuming stochastic continuity of $T_a(w)$ and $T_0(w)$ in $(0,1)$, ie assuming the probability of a jump discontinuity of $T_0(w)$ at $w_0$ is 0, we will prove the continuity of $AUC(w)$.

Let $T(w)$ be  a stochastic process indexed by $w$ in continuous interval (0,1). Assume  the process is continuous in probability, ie.
$$ \forall a  \lim_{\delta \rightarrow 0} Pr\left[\|T(w+\delta)-T(w) \|>a \right] \rightarrow 0$$
Now consider the event  $T(w)>0$ and $T(w+\delta)>0$ for any $\delta$. 
\begin{eqnarray}
\| Pr\left[T(w+\delta)>0 \right]- Pr\left[T(w+\delta)>0 \right]\| & = 
& \| Pr\left[T(w+\delta)>0   \cap T(w) \leq 0 \right]- Pr\left[T(w+\delta)\leq 0  \cap T(w) > 0\right]\| \\
&= &\| Pr\left[T(w+\delta)>0 - T(w) = \epsilon_1 > 0 \right]- Pr\left[ T(w)- T(w+\delta) = \epsilon_2 > 0   \right]\| \\
&= &\| Pr\left[T(w+\delta) - T(w) = a_1 > 0 \right]- Pr\left[ T(w)- T(w+\delta) = a_2 > 0   \right]\| \textrm{for some $a_1$ and $a_2$} \\
& <= & Pr\left[\|T(w+\delta) - T(w)\| = \max (a_1 ,a_2) >0 \right] \\
\end{eqnarray} 
 Let $a=\max (a_1 ,a_2)$. For any $a_1,a_2$, there exists a $\delta$ for which the probability of the event $Pr\left[\|T(w+\delta) - T(w)\right| > a$  is smaller than any given $\epsilon$ by the continuity in probability. The absolute difference of the two probabilities $\| Pr\left[T(w+\delta)>0 \right]- Pr\left[T(w+\delta)>0 \right]\| $ is also smaller than  this $\epsilon$ since it's upper bounded by the probability of that event.
Therefore $Pr\left[T(w)>0 \right]$  as a function of $w$ is continuous with respect to $w$. 
...
%It is possible this is not the global minimum in $\mathbb{R}^d$  
 The main assumption we used is the stochastic continuity of $T_0(w)$ and $T_a(w)$ . If  a jump discontinuity at $w$ under the probability distribution that governs the data has measure  zero, then this assumption holds. In the constructed example, we assumed a certain symmetry in the realized configuration, which gave us a discontinuity at a particular point of $w$. We assume any change in this symmetry will give a discontinuity at a different $w$. A further investigation should reveal if this holds.
 
 Another avenue for investigation is the question of how $w^{*}$ depends on the parameters of the distributions of $\mathbf{F}_m$ and  $\mathbf{F}_u$ . We aim to answer such questions by running simulations under Gaussian and Dirichlet settings. 
...
<<sim-two-cond-pre,fig=FALSE,echo=FALSE,results=hide,cache=FALSE,prefix.string="JOFCpaper",trace=TRUE>>=
rm(list=ls())
graphics.off()
...
<<wiki-two-cond, echo=FALSE,fig=FALSE,results=verbatim,cache=TRUE>>=
#,depenson=Init >>=
model= "gaussian"
...
run.wiki.JOFC.sim.mc.replicate <- function(m.i,N, test.samp.size, w.val.len, m, TE, TF,
	...
	D10A<- TE[train.test.0,train.test.0]
	D20<-  TF[train.test.0,train.test.0]
	
	D2A<-  TF[train.test.A,train.test.A]
...
<<wiki-two-cond-plot, echo=FALSE,fig=TRUE,results=hide,cache=FALSE,dependson=wiki-two-cond >>=
...
<<stats-d-sim-pre, echo=FALSE,fig=FALSE,results=hide,cache=FALSE >>=
rm(list=ls())
debug.mode<-FALSE
run.for.Sweave<-TRUE
load.project()
...
for (i in (1:length(d.vals))){
	...
	params.list[[i]]$d.param <- d.vals[i]
	params.list[[i]]$c.val.param <- 0.01
}

debug.mode <- FALSE
oos.stats.d<-tryCatch(compute.stats.for.params(params.list,"d"),error=function(e) {
			sink("debug.oos.stats.d.txt")
			print(traceback())
			print(e)			
			print(environment())
			#print(parent.env())
		    ...
		    e})
if (inherits(oos.stats.d, "try-error")){
	traceback()
} else{	
...
<<fig-stats-d, echo=FALSE,fig=TRUE,results=hide,cache=FALSE , dependson= stats-d-sim>>=
...
source("./src/runningParams.R",local=TRUE)
c.vals<-c(0.01)
...
vary.params<-TRUE
print(params)
short.w.vals <- c(0.5,0.8,0.925,0.99,0.999)
params$w.vals <-short.w.vals 

dirichlet.sim<-F
source("./src/GaussianSimFidCommTradeoffMain.R",local=TRUE)

@

<<FidComm-d-plot-Gaussian, echo=FALSE,fig=FALSE,results=verbatim,cache=FALSE,dependson=FidComm-d-sim >>=
run.for.Sweave<-TRUE
...
first.ind<-rep(0,length(d.vals))
for (i in 1: length(d.vals)){
	first.ind[i]<-which(params.df$d.i==d.vals[i])
}

sink("./logs/debug-fidcomm.txt")
 print(str(Fid1))
 #print(c("Fid1","Fid2","Comm"))
#print(cbind(Fid1,Fid2,Comm))
sink()
#def.w.vals <- c(0.5,0.8,0.85,0.9,0.925,0.95,0.99,0.999)
if( run.in.linux) X11() else {windows()}

plot.graph.with.CI(Fid1,"Fid and Comm Terms","red",conf.int=TRUE,   add=FALSE,fp.points=(1:ncol(Fid1)),
		customx.labels=NULL,customy.labels=NULL,ispowercurve=FALSE)

plot.graph.with.CI(Fid2,"Fid and Comm Terms","orange",conf.int=TRUE,add=TRUE,fp.points=(1:ncol(Fid1)),
		customx.labels=NULL,customy.labels=NULL,ispowercurve=FALSE)

plot.graph.with.CI(Comm,"Fid and Comm Terms","blue",conf.int=TRUE,  add=FALSE,fp.points=(1:ncol(Fid1)),
		customx.labels=NULL,customy.labels=NULL,ispowercurve=FALSE)


#fname<- file.path('graphs',paste(c(model,"-FidCommTerms-n",n,"-",ifelse(oos,"OOS","noOOS"),"c",params$c.val),collapse="",sep=""))
...
<<FidComm-d-plot-Gaussian-2, echo=FALSE,fig=TRUE,results=hide,cache=FALSE,dependson=FidComm-d-sim >>=
...
w.val.len<-length(short.w.vals)
for (w.i in 1:ncol(avg.Fid1.alt.G)){
	if (w.i==1) {
		plot (d.vals,avg.Fid1.alt.G[first.ind, w.i],type="l",col="red")
	} else {
		lines (d.vals,avg.Fid1.alt.G[first.ind, w.i],col="red")}
...
<<FidComm-d-plot-Gaussian-3, echo=FALSE,fig=TRUE,results=hide,cache=FALSE,dependson=FidComm-d-sim >>=
for (w.i in 1:ncol(avg.FCratios.List.G)){
...
<<d-ROC, echo=FALSE,fig=TRUE,results=hide,cache=FALSE,depenson= FidComm-d-sim,trace=TRUE >>=
lty.i.vec<-c()
lwd.i.vec <- c()
line.width<-1.5
w.val.len<- length(params$w.vals)
plot.w.vals<-short.w.vals
...
<<d-w-hist, echo=FALSE,fig=TRUE,results=hide,cache=FALSE,depenson= FidComm-d-sim,trace=TRUE >>=
d.vals.len<-length(d.vals)
par(mfrow=c(ceiling(d.vals.len/2),2))
for (i in 1:d.vals.len)
	hist(best.w.g.list.param.i,breaks=length(short.w.vals),
			main=expression(paste("Best w for d=",d.vals[i])))
